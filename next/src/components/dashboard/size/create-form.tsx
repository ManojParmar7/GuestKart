...
import { createSize, GetUsersBySuperadmin } from "../../../app/query-common";

export function CreateForm(): React.JSX.Element {
	const [CreateSize] = useMutation(createSize);
	...
	const handleSubmit = async (event: React.FormEvent) => {
		...
		if (validate()) {
			...
			try {
				...
				showToast({
					message: data?.createSize?.message || " created successfully.",
					type: "success",
				});
	...
	return (
		<form onSubmit={handleSubmit}>
			<Card elevation={3} sx={{ p: 2 }}>
				<CardHeader name="Create Size" subheader="Fill in the details to create a new Size" sx={{ mb: 2 }} />
				...
				<CardActions sx={{ justifyContent: "flex-end", gap: 2 }}>
					<Button variant="outlined" color="error" onClick={() => router.push(`/dashboard/size`)}>
						Cancel
					</Button>
