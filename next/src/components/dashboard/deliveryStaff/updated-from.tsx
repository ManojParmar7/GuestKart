...
import { getUserById, updateUser } from "../../../app/query-common";
...
export function UpdateForm(): React.JSX.Element {
	...
	const validateField = (name: string, value: string): string => {
		switch (name) {
			...
			case "phone": {
				return value.trim() ? (/^\+?[\d\s\-\(\)]+$/.test(value) ? "" : "Invalid phone format") : "Phone is required";
			}
			case "website": {
				return value.trim() ? (/^https?:\/\/.+\..+/.test(value) ? "" : "Invalid website URL") : "Website is required";
			}
			case "password": {
	...
	if (error) {
		return (
			<Alert
				...
				sx={{ mt: 2, p: 3 }}
				action={
					<Button color="inherit" size="small" onClick={() => router.push("/dashboard/deliveryStaff")}>
						Go Back
					</Button>
				}
			>
